...
def start(){
    try{
        sh """
         sed -i "s/<OUT_PORT>/${OUTPort}/" ${this.resourcePath}/*
         sed -i "s/<PRO_PORT>/${PROPort}/" ${this.resourcePath}/*
         sed -i "s/<JOB_NAME>/${PROJECT}/" ${this.resourcePath}/*
         sed -i 's#<IMAGE_URL>#${env.CURRENT_IMAGE}#g' ${this.resourcePath}/*
         kubectl apply -f ${this.resourcePath}      
       """
} catch (Exception exc){
...
def initWorkload() {
    try {
        ...
        println(this.workloadFilePath)
        //def content = new File(this.workloadFilePath).text
