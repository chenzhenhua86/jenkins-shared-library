...
def build() {
    ...
    retry(3) {
        try {
            sh """
              sed -i "s/<CURRENV>/${CurrEnv}/"  ${this.dockerfile}
              sed -i "s/<PRO_PORT>/${PROPort}/g"  ${this.dockerfile}
              sed -i "s/<JOB_NAME>/${PROJECT}/g"  ${this.dockerfile}
              cat Dockerfile
              docker build ${this.context} -t ${this.fullAddress} -f ${this.dockerfile}
            """
